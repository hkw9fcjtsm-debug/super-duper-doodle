...
// Map user selection to display label (PLACEHOLDERS you can rename)
...
function buildAccountBlocks(count, existingAccounts=null){
  ...
  for(let i=0;i<count;i++){
    ...
    // restore existing if provided
    if(existingAccounts && existingAccounts[i]){
      const a = existingAccounts[i];
      block.querySelector(".type").value = a.typeRaw;     // "Checking"/"Savings"
...
function collectAccounts(count){
  ...
  for(let i=0;i<count;i++){
    const b = blocks[i];
    const typeRaw = b.querySelector(".type").value; // Checking/Savings
    const last4Raw = b.querySelector(".last4").value.trim();
    const balRaw = b.querySelector(".balance").value.trim();

    let lastFour = last4Raw.toLowerCase() === "random" ? generateUniqueLast4(used) : last4Raw;
    ...
    accounts.push({
      typeRaw,
      typeDisplay: displayTypeFromSelect(typeRaw), // PLACEHOLDER labels
      lastFour,
      balanceCents: cents
    });
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("btnSignOff").addEventListener("click", () => {
    // placeholder action: just go back to input
    showInput();
  });
});
